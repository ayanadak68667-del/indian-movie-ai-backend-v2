...
router.post('/chat', async (req, res) => {
  try {
    ...
    // ✅ Strong input validation (quota save + safety)
    if (!message || message.trim().length < 2) {
      return res.status(400).json({ 
        success: false, 
        error: "Please write a valid movie-related message." 
      });
    }
...
`;
    ...
    res.status(500).json({ 
      success: false, 
      error: "🎬 Filmi AI is taking a short movie break! Please try again later." 
    });
  }
});
